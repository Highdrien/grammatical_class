...
class MorphLSTMClassifier(nn.Module):
    def __init__(self,
        ...
        for i in range(num_classes):
            self.morph.append(nn.Linear(in_features=num_c_possibility, out_features=num_c_possibility).to('cuda')) #chaque couche doit récupérer un object de taille 13
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.do_morphy:
            logits = logits.view(-1, sequence_length, self.num_classes, self.num_c_possibility)
            #we separate each class in a different Dense layer
            logit_list = []
            for i in range(self.num_classes):
                logits_i = self.morph[i](logits[:, :, i, :])
                logit_list.append(logits_i)
            #we concatenate all the classes to have a tensor of shape (B, K, C * num_c_possibility)
