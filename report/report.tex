...
L'accuracy micro est relativement meilleure que l'accuracy macro, ce qui signifie que les performances de toutes les classes confondues sont
meilleures que si on cherche à quantifier les performances globales à partir de chaque classe séparément, traitée de manière égale. D'autre part les performances d'accuracy macro sont moins stables que celle de l'accuracy micro. 
...
Néanmoins, l'accuracy micro est plus stable que celle de l'accuracy \textit{allgood} lorsqu'on considère les classes séparément dans le
calcul de l'accuracy, où l'écart entre validation et entraînement se creuse au fil des epochs. L'amélioration des performances
n'est donc pas égale au fil des dernières epochs entre les différentes classes. Le modèle a donc tendance à overfitter si on considère l'accuracy \textit{allgood}. 
